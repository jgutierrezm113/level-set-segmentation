...
void evolveContour(unsigned char* intensity, 
	...
	// Timing variables
	clock_t processing_tbegin;
	clock_t processing_tend;
	
	clock_t total_tbegin = clock();
	clock_t total_tend;
	
	clock_t initializing_tbegin = clock();
	clock_t initializing_tend;
	...
	initializing_tend = clock ();
	double elapsed_secs = double(initializing_tend-initializing_tbegin) / CLOCKS_PER_SEC;

        int numIterations = 0;
        stopCondition[j] = 1;
	
	processing_tbegin = clock();
	...
	processing_tend = clock ();
	elapsed_secs = double(processing_tend-processing_tbegin) / CLOCKS_PER_SEC;
	
	printf("Processing Time: %f\n", elapsed_secs);
	
	total_tend = clock ();
	elapsed_secs = double(total_tend-total_tbegin) / CLOCKS_PER_SEC;
